  ...
  return (
    <div className='border h-36 rounded flex w-full  '>
      <div className='w-56 min-w-[100px] bg-slate-300 rounded  box-border overflow-hidden'>
        <img
          className='w-full h-full p-3 box-border object-cover'
          src={img}
          alt={title}
        />
      </div>
      <div className='px-3 md:p-3 w-full min-w-210'>
        <div className='flex justify-between items-center'>
          <h1 className='text-[13px] mt-2 lg:text-xl md:whitespace-nowrap font-semibold text-slate-500'>
            {title}
          </h1>
          <div
            onClick={() => dispatch(deleteCartProduct(id))}
            className='text-slate-500 hover:text-white cursor-pointer hover:bg-red-600 p-1 rounded-full hover:scale-105 transition-all'>
            <MdOutlineDelete />
          </div>
        </div>
